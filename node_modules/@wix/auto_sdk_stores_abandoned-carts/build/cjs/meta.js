...
__export(meta_exports, {
  getAbandonedCart: () => getAbandonedCart2,
  queryAbandonedCarts: () => queryAbandonedCarts2
});
...
// src/stores-v1-abandoned-cart-abandoned-carts.meta.ts
function getAbandonedCart2() {
  const payload = { id: ":id" };
  const getRequestOptions = getAbandonedCart(payload);
...
function queryAbandonedCarts2() {
  const payload = {};
  const getRequestOptions = queryAbandonedCarts(payload);
  const getUrl = (context) => {
    const { url } = getRequestOptions(context);
    return url;
  };
